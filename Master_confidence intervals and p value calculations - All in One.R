...
SINGLE.MEAN.One.Sided.Test.T=function(observed.value, sample.mean, sd, n){  
  z=(observed.value-sample.mean)/(sd/sqrt(n))
  print(z) 
  p.value=1-pt(z, n-1)
  print(p.value)}

x=c(19,14,23,20,15,19,21,16,18,18,16,21)
SINGLE.MEAN.One.Sided.Test.T(observed.value=mean(x),sample.mean=14, sd=sd(x), n=12)
...
PROPORTION.One.Sided(observed.value=43/53, sample.prop=.66, n=53)
